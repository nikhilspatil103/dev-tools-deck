          ...
          <div className="jf__panel">
            <div className="jf__panel-header">
              ...
              <div className="jf__panel-actions">
                ...
                <button className="jf__panel-btn" onClick={() => { try { let s = input; const decodeHtml = (str) => str.replace(/&quot;/g, '"').replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&#39;/g, "'").replace(/&apos;/g, "'"); const decodeUnicode = (str) => str.replace(/\\u003c/gi, '<').replace(/\\u003e/gi, '>').replace(/\\u003d/gi, '=').replace(/\\u0026/gi, '&').replace(/\u003c/gi, '<').replace(/\u003e/gi, '>').replace(/\u003d/gi, '=').replace(/\u0026/gi, '&'); let prev = ''; while (s !== prev) { prev = s; s = decodeHtml(s); s = decodeUnicode(s); s = s.replace(/\\\\\*/g, m => m.slice(0, m.length/2)); } s = s.replace(/\\"/g, '"'); try { const parsed = JSON.parse(s); s = typeof parsed === 'string' ? parsed : JSON.stringify(parsed, null, 2); } catch {} setInput(s); } catch {} }} disabled={!input}>Unescape</button>
              </div>
          ...
          <div className="jf__panel">
            <div className="jf__panel-header">
              ...
              <div className="jf__panel-actions">
                ...
                <button className="jf__panel-btn" onClick={() => { try { const src = output || input; let s = src; const decodeHtml = (str) => str.replace(/&quot;/g, '"').replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&#39;/g, "'").replace(/&apos;/g, "'"); const decodeUnicode = (str) => str.replace(/\\u003c/gi, '<').replace(/\\u003e/gi, '>').replace(/\\u003d/gi, '=').replace(/\\u0026/gi, '&').replace(/\u003c/gi, '<').replace(/\u003e/gi, '>').replace(/\u003d/gi, '=').replace(/\u0026/gi, '&'); let prev = ''; while (s !== prev) { prev = s; s = decodeHtml(s); s = decodeUnicode(s); } s = s.replace(/\\"/g, '"'); try { const parsed = JSON.parse(s); s = typeof parsed === 'string' ? parsed : JSON.stringify(parsed, null, 2); } catch {} setOutput(s); } catch {} }} disabled={!output && !input}>Unescape</button>
